...
function init() {
	...
	$.post("../ajax/producto-all-store.php?op=selectLinea",
		function (values) {
			//    console.log(r);
			$("#idlinea").html(values);
		});
	...
	$("#idlinea").change(function () {
		...
		var idlinea = $(this).children("option:selected").val();

		$.post("../ajax/producto-all-store.php?op=selectSubLineaCbo&idlinea=" + idlinea,
			function (values) {
				$("#idproductosublinea").html(values);
				$('#idproductosublinea').selectpicker('refresh');
			});
	});
	//Cargamos los items al select linea
	/* 	$.post("../ajax/producto-all-store.php?op=selectLinea", function (r) {
			$("#idlinea").html(r);
			$('#idlinea').selectpicker('refresh');
		}); */
	//Cargamos los items al select sublinea
	/* $.post("../ajax/producto-all-store.php?op=selectSubLinea", function (r) {
		$("#idproductosublinea").html(r);
		$('#idproductosublinea').selectpicker('refresh');
	}); */
	//Cargamos los items al select unidad medida	
	$.post("../ajax/producto-all-store.php?op=selectUnidadMedida", function (r) {
		$("#idunidadmedida").html(r);
		$('#idunidadmedida').selectpicker('refresh');
	});
	//Cargamos los items al select estado	
	/* 	$.post("../ajax/producto-all-store.php?op=selectProductoEstado", function (r) {
			$("#idproductoestado").html(r);
			$('#idproductoestado').selectpicker('refresh');
		}); */
	//Cargamos los items al select anexo	
	$.post("../ajax/producto-all-store.php?op=selectAnexoProveedor", function (r) {
		$("#idanexo_proveedor").html(r);
		$('#idanexo_proveedor').selectpicker('refresh');
	});
	//Cargamos los items al select marca	
	$.post("../ajax/producto-all-store.php?op=selectMarca", function (r) {
		$("#idmarca").html(r);
		$('#idmarca').selectpicker('refresh');
	});

	$('#mAlmacen').addClass("treeview active");
	$('#lProductosAllStore').addClass("active");
...
function listar() {
	tabla = $('#tbllistado').dataTable(
		...
		"ajax": {
			url: '../ajax/producto-all-store.php?op=listar',
			type: "get",
			dataType: "json",
			error: function (e) {
...
function guardaryeditar(e) {
	...
	$.ajax({
		url: "../ajax/producto-all-store.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(idproducto) {
	$.post("../ajax/producto-all-store.php?op=mostrar", {
...
function desactivar(idproducto) {
	bootbox.confirm("¿Está Seguro de desactivar el producto?", function (result) {
		if (result) {
			$.post("../ajax/producto-all-store.php?op=desactivar", {
				idproducto: idproducto
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
...
function activar(idproducto) {
	bootbox.confirm("¿Está Seguro de activar el producto?", function (result) {
		if (result) {
			$.post("../ajax/producto-all-store.php?op=activar", {
				idproducto: idproducto
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
