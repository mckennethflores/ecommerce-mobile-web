...
function init() {
    ...
    $('#lPedidosGeneral').addClass("active");
...
function listar() {
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../ajax/pedidos.php?op=listarpedidosgeneral',
            type: "get",
            dataType: "json",
            error: function (e) {
...
function mostrar(idpedido, cliente) {
    $.post("../ajax/pedidos.php?op=mostrar", {
        ...
        $('#idusuario').selectpicker('refresh');
        var fecha = data.fechapedido;
        fechaFormateada=fecha.substring(0,10);
        $("#fechapedido").val(fechaFormateada);
