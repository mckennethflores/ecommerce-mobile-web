...
ob_start();
session_start();
require_once "../modelos/ProductoAllStore.php";

$producto = new ProductoAllStore();
    ...
    case 'guardaryeditar':
        ...
        if(empty($idproducto)){
            // $rspta=$producto->insertar($idproductosublinea, $idunidadmedida, $codproducto,$nomproducto,$nomproductocorto,$codigobarra,$imagen,$imagengrande,$idproductoestado,$idanexo_proveedor,$valorventa,$observacion,$idmarca,$preciopromocion,$stockproducto,$_SESSION['idtienda']);
                $rspta=$producto->insertar($idproductosublinea, $idunidadmedida,$nomproductocorto,$codigobarra,$imagen,$valorventa,$observacion,$idmarca,$preciopromocion,$stockproducto,$_SESSION['idtienda']);
            ...
            //  $rspta=$producto->editar($idproducto, $idproductosublinea, $idunidadmedida, $codproducto,$nomproducto,$nomproductocorto,$codigobarra,$imagen,$imagengrande,$idproductoestado,$idanexo_proveedor,$valorventa,$observacion,$idmarca,$preciopromocion,$stockproducto);
                $rspta=$producto->editar($idproducto, $idproductosublinea, $idunidadmedida,$nomproductocorto,$codigobarra,$imagen,$valorventa,$observacion,$idmarca,$preciopromocion,$stockproducto);
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>($reg->idproductoestado)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idproducto.')"><i class="fa fa-pencil"></i></button>'.
                    ...
                    "5"=>$reg->nomtienda,
                    "6"=>($reg->idproductoestado)?'<span class="label bg-green">Activado</span>':'<span class="label bg-red">Desactivado</span>'
                );
        }
        $results = array(
